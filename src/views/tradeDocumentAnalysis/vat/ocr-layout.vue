      ...
      <div class="document-box" ref="document-box">
        ...
        >
          ...
          >
            ...
            >
              <!-- 默认展示当前页所有可用框（浅色），不影响选中高亮与连线 -->
              <template v-for="(poly, pIndex) in allMaskPolygonList">
                <path
                  :key="`all-${pIndex}`"
                  stroke-width="0.5"
                  stroke="#009688"
                  ...
                  fill="rgba(8, 135, 255, 0.08)"
                  :d="multiMaskElPathValue(poly)"
                />
              </template>
...
export default {
  ...
  computed: {
    ...
    // 汇总当前页的所有可用 position 多边形，用于默认展示全部框
    allMaskPolygonList() {
      const result = [];
      const page = this.page || {};
      const content0 = page.content && page.content[0] ? page.content[0] : null;
      if (!content0) return result;
      const pushPosition = (pos) => {
        if (!Array.isArray(pos)) return;
        // pos 形如 [ [ {x,y}... ], [ {x,y}... ] ]，逐个多边形压入
        pos.forEach((poly) => {
      ...
      // info
      (content0.info || []).forEach((item) => {
        pushPosition(item && item.position);
      });
      // others
      (content0.others || []).forEach((item) => {
        pushPosition(item && item.position);
      });
      // commodity 为二维数组
      (content0.commodity || []).forEach((arr) => {
        (arr || []).forEach((item) => {
          pushPosition(item && item.position);
        });
      });
