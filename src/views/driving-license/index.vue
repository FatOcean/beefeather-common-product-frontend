<template>
  <div class="document-ocr-wrapper" :class="{ 'bee-loading': beeLoading }">
    ...
    <lls-collapse-transition v-if="pageMenuPerm['uploadDrivingLicense']">
...
export default {
  ...
  methods: {
    ...
    ocrRecognitionExcel(file) {
      this.$http
        .post(
          `/general-product-web/general/productRecognition?taskId=${this.files[0].taskId}&productName=驾驶证解析`
        )
        .then((res) => {
          res = res.data;
          if (res.code === "200") {
    ...
    handleClickDownload() {
      this.$http({
        method: "get",
        url: `/general-product-web/general/downloadResult?taskId=${this.$refs.ocrlayout.example.requestId}&productName=驾驶证解析`,
        responseType: "blob",
      })
        .then((res) => {
          const fileName =
