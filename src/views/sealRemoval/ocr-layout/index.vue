        ...
        <div class="example-image">
          <img
            :src="
              i.imageVO[0].image
                ? i.imageVO[0].image
                : originalLocation + encodeURIComponent(i.imageVO[0].filePath)
            "
            :alt="i.fileName"
          />
        </div>
        ...
        <div class="ocr-title-bar">
          ...
          <div class="function-bar">
            <lls-dropdown
              ...
              v-if="pageMenuPerm['DOWSEALREM']"
            >
              <llsButton type="text" class="lls-dropdown-link">
...
export default {
  ...
  methods: {
    ...
    reRenderImage() {
      ...
      this.newScale =
        page.realRenderHeight > 0.79 * baseHeight
          ? (0.79 * baseHeight) / page.realRenderHeight
          : 1;
    ...
    getBoundingClientRect() {
      return this.$refs["document-box"].getBoundingClientRect();
    },
    // 切换示例
    ...
    handleTurnPage(val) {
      ...
      if (num === this.total) {
        ...
        if (this.activeDocumentIndex === 0) {
          ...
          this.activePageIndex =
            this.data[this.activeDocumentIndex].imageVO.length / 2;
        } else {
          this.activeDocumentIndex -= 1;
          this.activePageIndex =
            this.data[this.activeDocumentIndex].imageVO.length / 2;
      ...
      const page = this.example.imageVO[this.activePageIndex - 1];
      this.resizeImg();
      this.$emit("handle-change", page);
      this.$emit("tabs", this.activeDocumentIndex, this.activePageIndex - 1);
  ...
  computed: {
    ...
    // 文档图片地址
    imageUrl() {
      return this.data[this.activeDocumentIndex].imageVO[0].image
        ? this.data[this.activeDocumentIndex].imageVO[0].image
        : this.originalLocation +
            encodeURIComponent(
              this.data[this.activeDocumentIndex].imageVO[0].filePath
            );
    },
    imageName() {
...
.lls-select .lls-input .lls-select__caret.is-reverse {
