  ...
  if ('setDragImage' in window.DataTransfer.prototype && document.body.animate) {
    ...
    document.addEventListener('dragstart', function (ev) {
      if (ev.target.nodeType === Node.ELEMENT_NODE) {
        if (!ev.target.classList.contains('image-box')) return
        dragbox = ev.target.parentElement
        dragbox.dragData = {}
        ...
        const { clientHeight, clientWidth } = dragbox
        fakeObj.style.width = clientWidth * transformImg + 'px'
        fakeObj.style.height = clientHeight * transformImg + 'px'
        ...
        cloneObj.style.width = clientWidth * transformImg + 'px'
        cloneObj.style.height = clientHeight * transformImg + 'px'
        // cloneObj.style.paddingTop = '12px';
        cloneObj.style.background = 'rgba(236,248,254,.6)'
        cloneObj.style.borderRadius = '12px'

        if (ev.frontIndex > 1) {
          const countBadge = document.createElement('div')
          countBadge.textContent = ev.frontIndex
          countBadge.className = 'drag-count-badge'
          cloneObj.appendChild(countBadge)
        }
